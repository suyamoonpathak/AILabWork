...
def best(cur,completed):
    ...
    if "right" in possibleMoves:
        ...
        if (copyCurRight not in visited):
            dfsQueue.append((copyCurRight, f))
        else:
            print("not adding this because already visited!!!")
    ...
    # BFS
    return dfsQueue[len(dfsQueue)-1][0],completed
...
if __name__ == "__main__":
    ...
    # while (steps):
        ...
        if (completed):
            print("\n\n")
            print("SOLUTION : ")
            printLs(dfsQueue[len(dfsQueue)-1][0])
            break
        steps -= 1
